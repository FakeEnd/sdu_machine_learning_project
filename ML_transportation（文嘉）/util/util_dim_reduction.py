...
# %configur InlineBackend.figure_format = "svg"

'''
该模块用于对数据如embedding进行降维可视化
'''
    ...
    if data_label:
        for i in range(len(X_pca)):
            plt.annotate(data_label[i], xy=(X_pca[:, 0][i], X_pca[:, 1][i]),
                         xytext=(X_pca[:, 0][i] + 0.00, X_pca[:, 1][i] + 0.00))  # 这里xy是需要标记的坐标，xytext是对应的标签坐标
    ...
    if data_label:
        for i in range(len(X_tsne)):
            plt.annotate(data_label[i], xy=(X_tsne[:, 0][i], X_tsne[:, 1][i]),
                         xytext=(X_tsne[:, 0][i] + 1, X_tsne[:, 1][i] + 1))  # 这里xy是需要标记的坐标，xytext是对应的标签坐标
    plt.title(title, fontdict=font)
